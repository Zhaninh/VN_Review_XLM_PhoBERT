...
MODEL_PATH = r"D:\FSoft\Review_Ana\Dream_Tim\A\weights\model.pt"
...
async def analyse_review(review_data: Review):
    review_sentence = tuple(review_data.text)
    # Create a dictionary with your review
    ...
    prep = preprocess()
    # Preprocess the data
    preprocessed_data = prep.run(DatasetDict({"test": input_data}))

    # Extract the preprocessed review
    inputt = preprocessed_data["test"]["Review"]

    with torch.no_grad():
        outputs_classifier, outputs_regressor = model(**inputt)
